...
SHUTDOWN_THRESHOLD = 2  # Number of consecutive failures required for shutdown
SLEEP_TIME = 20  # Time in seconds to wait between failure checks
...
try:
    ...
    while True:
        ...
        for _ in range(SHUTDOWN_THRESHOLD):
            ...
            if ac_power_state == 0:
                print(f"UPS is unplugged or AC power loss detected - using battery power")
                failure_counter += 1
            else:
                failure_counter = 0
                break
        ...
        if failure_counter >= SHUTDOWN_THRESHOLD:
            print(f"Failure threshold reached - shutdown once battery threshold of {BATTERY_THRESHOLD}% reached")
            if readCapacity(bus) < BATTERY_THRESHOLD:
                shutdown_reason = "due to loss of AC power and battery threshold reached"
